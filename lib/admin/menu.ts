...
export async function mutateMenu(db: PrismaClient, value: unknown, create: boolean) {
  ...
  const { entity, id, name, priceInAgorot, isAvailable, archived, toppingIds } = result.data;
  if ((create && (id || name === undefined || priceInAgorot === undefined || archived !== undefined)) ||
      (!create && (!id || Object.keys(result.data).length <= 2)) || (entity === 'topping' && toppingIds !== undefined)) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      return await db.$transaction(async (tx) => {
        ...
        if (entity === 'item') {
          const item = create
            ? await tx.menuItem.create({ data: { ...data, name: name!, priceInAgorot: priceInAgorot! } })
          ...
          if (toppingIds !== undefined) {
            ...
            if (toppingIds.length) await tx.menuItemTopping.createMany({ data: toppingIds.map((toppingId) => ({ menuItemId: item.id, toppingId })) });
          }
          return { id: item.id };
        }
